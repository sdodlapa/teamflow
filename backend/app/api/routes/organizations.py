...
from app.schemas.organization import (OrganizationCreate, OrganizationList,
...
async def list_my_organizations(
    ...
    limit: int = Query(
        20, ge=1, le=100, description="Number of organizations to return"
    ),
...
async def add_organization_member(
    ...
    # Add member
    member = OrganizationMember(
        organization_id=org_id, user_id=user.id, role=member_data.role
    )
    db.add(member)
    await db.commit()
...
async def remove_organization_member(
    ...
    if (
        ...
        and member.role == OrganizationMemberRole.OWNER
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
